...
class Skeleton:
    ...
    def distance(self, ske):        # TP-TODO
        ...
        For this TP we mainly care about pose similarity in 2D, so we compute an L2 distance
        on the *reduced* skeleton (13 joints in 2D => 26 values). This is faster and usually
        more stable than summing 3D landmark norms.
        """
        a = self.__array__(reduced=True).astype(np.float32).reshape(-1)
        b = ske.__array__(reduced=True).astype(np.float32).reshape(-1)
        # If mediapipe fails or shapes differ, fall back to a large distance
        if a.shape != b.shape:
            return float('inf')
